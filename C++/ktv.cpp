// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  PHANTOM  —  Terminal Music Player + Karaoke Engine  v4.0  (Linux, C++17)   ║
// ║  Features: Karaoke lyrics sync, spectrum visualizer, warp overlay,          ║
// ║            full playlist management, bookmarks, stats, genre filter          ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
...
// ═══════════════════════════════════════════════════════════════
//  ANSI palette — rich greyscale + accent colours
// ═══════════════════════════════════════════════════════════════
namespace A {
    // Reset & effects
    ...
    // Accent colours (karaoke, highlights)
    ...
    inline std::string saveCur(){ return "\033[s"; }
    inline std::string rstCur() { return "\033[u"; }
}
...
// Visible length (strips ANSI escapes, counts UTF-8 codepoints)
static int visLen(const std::string& s) {
    int len = 0;
    bool inEsc = false;
    for (size_t i = 0; i < s.size(); ) {
        unsigned char c = (unsigned char)s[i];
        if (c == '\033') { inEsc = true; ++i; continue; }
        if (inEsc) { if (std::isalpha(c)) inEsc = false; ++i; continue; }
        if ((c & 0xC0) != 0x80) ++len; // leading byte of UTF-8 codepoint
        ++i;
    }
    return len;
}

// Pad right to visual width w (truncate with ... if needed)
static std::string padR(const std::string& s, int w) {
    ...
    if (vl > w) {
        ...
        out += A::R;
        out += "...";
        out += std::string((size_t)std::max(0, w - vis - 3), ' ');
        return out;
    }
...
static std::string starStr(int r) {
    std::string s;
    for (int i = 1; i <= 5; i++) s += (i <= r ? "★" : "☆");
    return s;
}

// ═══════════════════════════════════════════════════════════════
//  Box drawing helpers  (proper ║ alignment)
// ═══════════════════════════════════════════════════════════════
...
static std::string boxMid(int w)     { return "║" + rep(" ", w - 2) + "║"; }

// Render a box row: ║<space><content padded to w-4><space>║
static std::string boxRow(const std::string& content, int w) {
    ...
    if (vl <= avail) {
        ...
        // Truncate at avail visible chars
        int vis = 0; bool inEsc = false;
        ...
        inner += A::R;
        inner += std::string((size_t)std::max(0, avail - vis), ' ');
    }
...
// Smooth sine-wave spectrum (multi-octave)
static std::string spectrumBar(int tick, int width, bool playing) {
    ...
    for (int i = 0; i < width; i++) {
        ...
        // Three-octave mix for realistic spectrum shape
        float v = 0.4f * std::sin(x * 6.28f * 2.1f + t * 1.3f)
                + 0.3f * std::sin(x * 6.28f * 5.3f + t * 2.1f + 1.0f)
                + 0.2f * std::sin(x * 6.28f * 11.7f + t * 3.7f + 2.0f)
                + 0.1f * std::sin(x * 6.28f * 23.0f + t * 5.1f + 0.5f);
        // Bell envelope: spectrum peaks in middle
...
//   Lines are stored sorted by time.
//   Active line is highlighted + centred; adjacent lines shown dim.
// ═══════════════════════════════════════════════════════════════
struct LyricLine {
    int    timeSec  = 0;   // start time in seconds
    int    timeMs   = 0;   // full time in ms (for sub-second precision)
    std::string text;
};

// Parse a single LRC timestamp "[mm:ss.xx]" → ms
static int parseLRCTime(const std::string& tok) {
    // tok looks like "01:23.45"
    int m = 0, s = 0, cs = 0;
...
static std::vector<LyricLine> parseLRC(const std::string& lrc) {
    ...
    while (std::getline(ss, line)) {
        ...
        while (pos < line.size() && line[pos] == '[') {
            ...
            // Check if it's a time tag (starts with digit)
...
// Find active lyric line index for given posMs
static int activeLyric(const std::vector<LyricLine>& lines, int posMs) {
    if (lines.empty()) return -1;
    int best = 0;
    for (int i = 0; i < (int)lines.size(); i++) {
        if (lines[i].timeMs <= posMs) best = i;
        else break;
    }
    return best;
}

// Built-in demo lyrics for the seed songs (LRC format strings)
...
static void initBuiltinLyrics() {
    ...
    gBuiltinLyrics["Comfortably Numb"] =
        ...
        "[02:18.00]There'll be no more\n"
        "[02:21.00]Ah, but you may feel a little sick\n"
    ...
    gBuiltinLyrics["Echoes"] =
        ...
        "[03:46.00]And help me understand\n"
        "[03:54.00]The best I can?\n"
        "[04:05.00]♪ Instrumental passage ♪\n"
        ...
        "[18:00.00]Cloudless every day you fall\n"
        "[18:10.00]Upon my waking eyes\n"
        "[18:20.00]Inviting and inciting me\n"
        "[18:30.00]To rise\n"
        "[19:00.00]And through the window in the wall\n"
        "[19:10.00]Come streaming in on sunlight wings\n"
        "[19:20.00]A million bright ambassadors of morning\n"
        "[21:00.00]♪ Final movement ♪\n"
    ...
    gBuiltinLyrics["Black"] =
        ...
        "[00:52.00]But I know in part\n"
        "[00:57.00]She's every inch the woman\n"
        "[01:02.00]That I ever dreamed she'd be\n"
        "[01:10.00]And all I taught her was\n"
        "[01:14.00]Everything I know\n"
        "[01:19.00]Oh, I know she gave me all that she wore\n"
        "[01:26.00]And now my bitter hands\n"
        "[01:31.00]Chafe beneath the clouds\n"
        "[01:36.00]Of what was everything\n"
        "[01:41.00]Oh, the pictures have\n"
        "[01:45.00]All been washed in black\n"
...
public:
    ...
    void shuffle() {
        ...
        for (int i = (int)arr.size() - 1; i > 0; i--) {
            int j = rand() % (i + 1); std::swap(arr[i], arr[j]);
        }
    ...
    std::vector<Node*> byGenre(const std::string& genre) const {
        ...
        Node* n = head_;
        do { if (lo(n->song.genre) == gl) res.push_back(n); n = n->next; } while (n != head_);
        return res;
    }
    ...
    int totalPlayCount() const {
        ...
        if (!head_) return 0;
        Node* n = head_;
        do { t += n->song.playCount; n = n->next; } while (n != head_);
        return t;
    }
    int totalDurationSec() const {
        ...
        if (!head_) return 0;
        Node* n = head_;
        do { t += n->song.toSecs(); n = n->next; } while (n != head_);
        return t;
    }
    ...
    void calculateStats() {
        ...
        if (head_) {
            Node* n = head_;
            do {
                ap[n->song.artist] += n->song.playCount; tp += n->song.playCount;
                sp[n->song.title + " - " + n->song.artist] += n->song.playCount;
                gp[n->song.genre] += n->song.playCount;
                if (n->song.rating > 0) { tr += n->song.rating; rs++; }
                n = n->next;
            } while (n != head_);
        }
        ...
        avgPlaysPerSong_ = size_ > 0 ? (double)tp / size_ : 0;
        avgRating_       = rs > 0 ? (double)tr / rs : 0;
        // Vibe
        size_t ua = ap.size(), ug = gp.size();
        if (size_ == 0)            listeningVibe_ = "The Silent Listener — start adding songs!";
        else if ((double)ua/size_ > 0.7 && (double)ug/size_ > 0.5)
        ...
        else if (avgRating_ > 4.0) listeningVibe_ = "The Connoisseur — only the finest tracks!";
        else if (totalMinutes_ > 10000)
                                   listeningVibe_ = "The Audiophile — music is life!";
        else if (ug <= 2)          listeningVibe_ = "The Specialist — you know what you like!";
        else {
            const char* v[] = {
...
private:
    ...
    void pushHistory() {
        ...
        char buf[16]; strftime(buf, sizeof(buf), "%H:%M:%S", tm_);
        HistEntry e;
    ...
    void freeList() {
        if (!head_) return;
        Node* n = head_; Node* nxt;
        do { nxt = n->next; delete n; n = nxt; } while (n != head_);
...
// ═══════════════════════════════════════════════════════════════
//  UI  —  full-screen TUI with karaoke engine
// ═══════════════════════════════════════════════════════════════
class UI {
public:
    enum class Mode { MAIN, KARAOKE, WARP };

    explicit UI(MusicPlayer& mp) : mp_(mp) {}

    void draw() {
        if (gResized) { gResized = 0; updateTermSize(); }
        // Double-buffer: build entire frame in a string, flush once
        buf_.clear();
        buf_ += A::go(1, 1);  // Home position

        if      (mode_ == Mode::KARAOKE) renderKaraoke();
        else if (mode_ == Mode::WARP)    { renderMain(); renderWarpOverlay(); }
        else                              renderMain();
    ...
    void commitWarp() {
        ...
        } catch (...) { setStatus("[!] Invalid position.", A::RED); }
        warpInput_.clear();
        mode_ = Mode::MAIN;
    }
    ...
    // ── Modals ────────────────────────────────────────────────
    void modalAdd() {
        rawClear(); modalHeader("ADD SONG");
        Song s; s.addedDate = mp_.today();
        s.title  = fp("  Title              ");
        if (s.title.empty())  { setStatus("[!] Title required.", A::RED); return; }
        s.artist = fp("  Artist             ");
        if (s.artist.empty()) { setStatus("[!] Artist required.", A::RED); return; }
        s.genre  = fp("  Genre              ");
        ...
        try { s.rating = std::max(0, std::min(5, std::stoi(rat))); } catch (...) {}
        if (mp_.insertBack(s)) {
            setStatus("[+] Added: " + s.title + " - " + s.artist, A::GRN);
            mp_.saveCSV();
        } else { setStatus(mp_.lastMsg(), A::RED); mp_.clearMsg(); }
    }

    void modalEdit() {
        const Song* s = mp_.currentSong();
        if (!s) { setStatus("[!] No song.", A::RED); return; }
        rawClear(); modalHeader("EDIT: " + s->title);
    ...
    void modalDelete() {
        const Song* s = mp_.currentSong();
        if (!s) { setStatus("[!] No song.", A::RED); return; }
        rawClear(); modalHeader("DELETE SONG");
        std::cout << A::W << "\n  " << s->title << " - " << s->artist << "\n"
    ...
    void modalPlaylist() {
        ...
        while (true) {
            ...
            modalHeader("PLAYLIST  [j/k=scroll · q=back]");
            ...
            if (k == 'k' || k == 'K') { if (offset > 0) --offset; }
            if (k == 'j' || k == 'J') { if (offset + visible < total) ++offset; }
            if (k == 27) {
                int k2 = readKey();
                if (k2 == '[') {
                    int k3 = readKey();
                    if (k3 == 'A' && offset > 0) --offset;
                    if (k3 == 'B' && offset + visible < total) ++offset;
                }
            }
        }
    }
    ...
    void modalHistory() {
        rawClear(); modalHeader("PLAYBACK HISTORY");
    ...
    void modalStats() {
        rawClear(); modalHeader("STATISTICS DASHBOARD");
        std::cout << "\n";
    ...
    void modalInfo() {
        const Song* s = mp_.currentSong();
        rawClear(); modalHeader("SONG DETAILS");
        if (!s) {
            ...
            row("Lyrics",     hasLyrics ? "Available — press [c] to enter Karaoke" : "None (add .lrc file)");
        }
        waitPrompt();
    }

    void modalSort() {
        rawClear(); modalHeader("SORT PLAYLIST");
        std::cout << A::MG
            << "\n  [1]  Title (A-Z)\n  [2]  Artist (A-Z)\n  [3]  Genre\n"
            << "  [4]  Year (oldest first)\n  [5]  Most Played\n  [6]  Highest Rated\n"
    ...
    void modalGenreFilter() {
        rawClear(); modalHeader("GENRE FILTER");
        ...
        if (idx >= 0 && idx < (int)genres.size()) {
            ...
            rawClear(); modalHeader("GENRE: " + genres[idx]);
            hline();
            for (Node* n : results) {
                bool ic = (n == mp_.currentNode());
    ...
    void modalBookmarks() {
        rawClear(); modalHeader("BOOKMARKS  [a=add · d#=del · j#=jump · q=back]");
        ...
        if (cmd == "a" || cmd == "A") {
            ...
            int i = -1; try { i = std::stoi(cmd.substr(1)) - 1; } catch (...) {}
            if (i >= 0 && i < (int)bms.size()) { mp_.removeBookmark(i); setStatus("Bookmark removed."); }
        } else if (!cmd.empty() && cmd[0] == 'j') {
            int i = -1; try { i = std::stoi(cmd.substr(1)) - 1; } catch (...) {}
            if (i >= 0 && i < (int)bms.size()) { mp_.jumpToBookmark(i); setStatus("Jumped to: " + bms[i].label, A::CYN); }
        }
    }

    void modalRate() {
        rawClear(); modalHeader("RATE CURRENT SONG");
        const Song* s = mp_.currentSong();
        ...
        try { mp_.rateCurrent(std::stoi(r)); mp_.saveCSV(); setStatus("Rated: " + starStr(mp_.currentSong()->rating), A::BYEL); }
        catch (...) { setStatus("[!] Invalid rating.", A::RED); }
    }

    void modalHelp() {
        rawClear(); modalHeader("PHANTOM — KEYBOARD SHORTCUTS");
        ...
        std::cout << "\n";
        for (int i = 0; h[i].k; i++)
            std::cout << A::BCYN << "  " << padR(h[i].k, 12) << A::MG << h[i].d << A::R << "\n";
        waitPrompt();
    }
...
private:
    ...
    std::string  buf_;   // double-buffer string
    ...
    bool statusExpired() {
        ...
        double e = (now.tv_sec - statusSetAt_.tv_sec)
    ...
    void outln(const std::string& s = "") { buf_ += s; buf_ += "\n"; }

    // Full line (for modal/non-buffered use)
    void hline(char ch = '-') {
    ...
    void renderKaraoke() {
        const Song* s = mp_.currentSong();
        buf_ += "\033[2J"; // clear screen

        // ── Header ────────────────────────────────────────────
        buf_ += A::go(1, 1);
        buf_ += A::MG; buf_ += "╔"; buf_ += rep("═", gCols - 2); buf_ += "╗\n";

        // Title line
        std::string titleStr = s ? (std::string(A::BCYN) + s->title + A::MG + " — " + A::CYN + s->artist) : "No song";
        buf_ += "║ ";
        buf_ += titleStr;
        int tl = visLen(titleStr);
        buf_ += std::string((size_t)std::max(0, gCols - 4 - tl), ' ');
        buf_ += A::MG; buf_ += " ║\n";

        // Progress + time on same line
        int el = mp_.elapsedSec(), tot = s ? s->toSecs() : 0;
        std::string elStr = fmtTime(el), totStr = fmtTime(tot);
        int barW = gCols - 4 - (int)elStr.size() - (int)totStr.size() - 4;
        if (barW < 4) barW = 4;
        std::string bar = pbar(mp_.progress(), barW);
        buf_ += "║ ";
        buf_ += A::MG; buf_ += elStr; buf_ += "  ";
        buf_ += A::CYN; buf_ += bar; buf_ += A::R;
        buf_ += A::MG; buf_ += "  "; buf_ += totStr;
        buf_ += std::string((size_t)std::max(0, gCols - 4 - (int)elStr.size() - 2 - barW - 2 - (int)totStr.size()), ' ');
        buf_ += A::MG; buf_ += " ║\n";
        buf_ += A::MG; buf_ += "╠"; buf_ += rep("═", gCols - 2); buf_ += "╣\n";

        // ── Lyrics area ───────────────────────────────────────
        int lyricsRows = gRows - 10;  // rows for lyrics display
        if (lyricsRows < 3) lyricsRows = 3;
        ...
        if (lines.empty()) {
            // No lyrics — show "wave" art
            for (int r = 0; r < lyricsRows; r++) {
                buf_ += A::MG; buf_ += "║ ";
                if (r == lyricsRows / 2) {
                    std::string msg = "♪ No lyrics available for this track ♪";
                    int ml = visLen(msg);
                    int pad = (gCols - 4 - ml) / 2;
                    buf_ += std::string((size_t)std::max(0, pad), ' ');
                    buf_ += A::DG; buf_ += msg; buf_ += A::MG;
            ...
            // Show context: (lyricsRows-1)/2 lines before, active, (lyricsRows-1)/2 after
            ...
            for (int row = 0; row < lyricsRows; row++) {
                ...
                if (li >= 0 && li < total) {
                    ...
                    std::string text = lines[li].text;
                    int tl2 = visLen(text);
                    int pad = (gCols - 4 - tl2) / 2;
                    if (pad < 0) pad = 0;

                    buf_ += std::string((size_t)pad, ' ');

                    if (isActive) {
                        // Active line: bold yellow with cursor effect
                        buf_ += A::BYEL;
                        // Karaoke character highlight (blinking cursor at word boundary)
                        buf_ += A::BOLD;
                        buf_ += text;
                        buf_ += A::R;
                    } else if (isNext) {
                        buf_ += A::W; buf_ += text; buf_ += A::R;
                    } else if (dist <= 2) {
                        buf_ += A::LG; buf_ += text; buf_ += A::R;
                    } else {
                        buf_ += A::MG; buf_ += text; buf_ += A::R;
                    }

                    int used = pad + tl2;
                    buf_ += std::string((size_t)std::max(0, gCols - 4 - used), ' ');
                } else {
                    buf_ += std::string((size_t)(gCols - 4), ' ');
                }
                buf_ += A::MG; buf_ += " ║\n";
        ...
        // ── Spectrum at bottom of karaoke ─────────────────────
        int specW = gCols - 4;
        std::string spec = spectrumBar(mp_.tick(), specW, mp_.isPlaying());
        buf_ += A::MG; buf_ += "║ ";
        buf_ += A::DIM; buf_ += A::CYN; buf_ += spec; buf_ += A::R;
        buf_ += A::MG; buf_ += " ║\n";

        buf_ += A::MG; buf_ += "╠"; buf_ += rep("═", gCols - 2); buf_ += "╣\n";

        // ── Controls row ──────────────────────────────────────
        std::string ctrls = "  SPC:play  ←→:skip  ↑↓:vol  f/B:seek  c:exit-karaoke  q:quit";
        buf_ += "║";
        buf_ += A::DG;
        buf_ += padR(ctrls, gCols - 2);
        buf_ += A::MG;
        buf_ += "║\n";
        buf_ += "╚"; buf_ += rep("═", gCols - 2); buf_ += "╝\n";
        buf_ += A::R;
    }
    ...
    void renderTopBar() {
        ...
        int used = visLen(appName) + visLen(dateStr) + visLen(trkStr);
        int gap1 = (gCols - 2 - used) / 2; if (gap1 < 1) gap1 = 1;
        int gap2 = gCols - 2 - visLen(appName) - gap1 - visLen(dateStr) - visLen(trkStr);
        if (gap2 < 1) gap2 = 1;

        buf_ += A::MG; buf_ += "║";
        buf_ += A::BCYN; buf_ += appName;
        buf_ += std::string((size_t)gap1, ' ');
        buf_ += A::DG;   buf_ += dateStr;
        buf_ += std::string((size_t)gap2, ' ');
        buf_ += A::MG;   buf_ += trkStr;
        buf_ += A::MG; buf_ += "║\n";
    ...
    void renderNowPlaying() {
        ...
        // Line 1: play state + title + elapsed
        buf_ += A::MG; buf_ += "║ ";
        if (!s) {
            std::string msg = "  No song loaded — press [a] to add songs";
            buf_ += A::MG; buf_ += padR(msg, gCols - 4);
        } else {
            std::string icon = mp_.isPlaying()
                ? std::string(A::BCYN) + "▶ "
                : std::string(A::MG)   + "⏸ ";
            std::string elStr = A::MG + fmtTime(mp_.elapsedSec());
            int titleW = gCols - 4 - 2 - visLen(fmtTime(mp_.elapsedSec())) - 1;
            if (titleW < 10) titleW = 10;
            buf_ += icon;
            buf_ += A::BW; buf_ += padR(s->title, titleW);
            buf_ += A::MG; buf_ += " "; buf_ += fmtTime(mp_.elapsedSec());
        }
        buf_ += A::MG; buf_ += " ║\n";
        ...
        if (s) {
            buf_ += A::MG; buf_ += "  ";
            buf_ += A::CYN; buf_ += padR(s->artist, 22);
            buf_ += A::DG;  buf_ += padR(s->genre, 14);
            std::string yearStr = s->year > 0 ? std::to_string(s->year) : "    ";
            buf_ += yearStr;
            buf_ += "  ";
            buf_ += A::BYEL; buf_ += starStr(s->rating); buf_ += A::R;
            int used = 2 + 2 + 22 + 14 + (int)yearStr.size() + 2 + visLen(starStr(s->rating));
            if (!s->tags.empty()) {
                std::string tagStr = "  #" + s->tags;
                int tagSpace = gCols - 4 - used;
                if (tagSpace > 4) {
                    if (visLen(tagStr) > tagSpace) tagStr = padR(tagStr, tagSpace);
                    buf_ += A::MG; buf_ += tagStr;
                    used += visLen(tagStr);
                }
            }
            int rem = gCols - 4 - used;
            if (rem > 0) buf_ += std::string((size_t)rem, ' ');
        } else {
            buf_ += std::string((size_t)(gCols - 4), ' ');
        }
        buf_ += A::R;
    ...
    void renderTransportRow() {
        std::string volBar = A::CYN + vbar(mp_.volume(), 12) + A::R;
        ...
        buf_ += A::MG; buf_ += "║ ";
        buf_ += A::MG; buf_ += "VOL["; buf_ += volBar; buf_ += A::MG; buf_ += "] ";
        buf_ += A::DG; buf_ += volPct; buf_ += "  ";
        buf_ += A::MG; buf_ += rpt; buf_ += "  ";
        ...
        if (!statusMsg_.empty() && statusW > 0) {
            if (statusExpired()) {
                ...
                if (inEsc) disp += A::R;
                buf_ += disp;
                buf_ += std::string((size_t)std::max(0, statusW - vis), ' ');
            }
        } else {
            if (statusW > 0) buf_ += std::string((size_t)statusW, ' ');
        }
        buf_ += A::R;
        buf_ += A::MG; buf_ += " ║\n";
    }
    ...
    void renderPlaylist() {
        // Column header
        buf_ += A::MG;
        buf_ += "  " + padR("#", 5) + padR("TITLE", 28) + padR("ARTIST", 20)
             +  padR("DUR", 7) + "STARS  PLAYS\n";
        buf_ += A::R;
        bDiv();
        ...
        do {
            if (idx >= offset && idx < offset + available) {
                ...
                std::string row = "  " + numStr + padR(n->song.title, 28) + padR(n->song.artist, 20)
                                ...
                                + "  " + std::to_string(n->song.playCount);
                if (ic) {
                    buf_ += A::INV; buf_ += row; buf_ += A::R; buf_ += "\n";
                } else if (idx % 2 == 0) {
                    buf_ += A::LG; buf_ += row; buf_ += A::R; buf_ += "\n";
                } else {
                    buf_ += A::MG; buf_ += row; buf_ += A::R; buf_ += "\n";
                }
            }
            n = n->next; ++idx;
        } while (n != mp_.headNode());
        ...
        buf_ += A::MG;
        buf_ += "  " + std::to_string(total) + " songs  |  #" + std::to_string(curIdx + 1)
             +  "  " + std::to_string(pct) + "%\n";
        buf_ += A::R;
    }
    ...
    void renderFooter() {
        ...
        if ((int)keys.size() > gCols - 2) keys = keys.substr(0, (size_t)(gCols - 2));
        buf_ += "║"; buf_ += A::DG; buf_ += padR(keys, gCols - 2); buf_ += A::MG; buf_ += "║\n";
        buf_ += "╚"; buf_ += rep("═", gCols - 2); buf_ += "╝\n";
        buf_ += A::R;
    }

    // ── WARP overlay (centered box over main screen) ──────────
    void renderWarpOverlay() {
        ...
        auto emit = [&](int row, const std::string& content) {
            buf_ += A::go(row, c0);
            buf_ += A::MG; buf_ += content; buf_ += A::R;
        };

        int row = r0;
        emit(row++, boxTop(bw));
        emit(row++, boxRow(std::string(A::BCYN) + "  ♪ PHANTOM WRAPPED — YOUR LISTENING STATS", bw));
        emit(row++, boxRow("", bw));
            ...
            if (v.empty()) {
                ...
                for (size_t i = 0; i < v.size() && i < 3; i++) {
                    std::string s = "  " + std::to_string(i + 1) + ". "
                                  + padR(v[i].first, 24) + " " + std::to_string(v[i].second) + " plays";
        ...
        emit(row++, boxRow(std::string(A::BYEL) + "  TOP SONGS", bw));
        {
            ...
            if (v.empty()) {
                ...
                for (size_t i = 0; i < v.size() && i < 3; i++) {
                    std::string s = "  " + std::to_string(i + 1) + ". "
                                  + padR(v[i].first, 30) + " " + std::to_string(v[i].second) + "×";
                    emit(row++, boxRow(std::string(A::LG) + s, bw));
                }
            }
        }
            ...
            if (v.empty()) {
                ...
                for (size_t i = 0; i < v.size() && i < 3; i++) {
                    ...
                    std::string s = "  " + padR(v[i].first, 14)
                                  + A::CYN + rep("█", filled) + A::MG + rep("░", barW2 - filled)
                                  + " " + A::LG + std::to_string(v[i].second);
        ...
        {
            char cbuf[80];
            std::snprintf(cbuf, sizeof(cbuf), "  Avg plays/song: %.1f  |  Avg rating: %.1f★  |  %d min listened",
        ...
        // Seek input
        emit(row++, boxRow(std::string(A::LG) + "  Seek to % (0-100):  "
                         + A::W + warpInput_ + A::BCYN + "█", bw));
        emit(row++, boxRow(std::string(A::DG) + "  Enter=jump · Esc=cancel  [w=toggle]", bw));
        emit(row,   boxBot(bw));

        buf_ += A::R;
    }

    // ── Modal helpers (non-buffered, use stdout directly) ─────
    static void rawClear() {
        std::cout << A::clrScr() << std::flush;
    }
...
int main(int argc, char* argv[]) {
    ...
    // Register SIGWINCH for terminal resize
    struct sigaction sa{};
    ...
    if (mp.dupesOnLoad() > 0)
        ui.setStatus("[!] " + std::to_string(mp.dupesOnLoad()) + " dupe(s) skipped on load.", A::YEL);
    ...
    while (!quit) {
        ...
        if (ui.mode() == UI::Mode::WARP) {
            if (k == 27 || k == 'q')           { ui.setMode(UI::Mode::MAIN); ui.warpInput().clear(); }
                ...
                if (ui.warpInput().size() < 6) ui.warpInput() += (char)k;
            } else if (k == 'w' || k == 'W')   { ui.setMode(UI::Mode::MAIN); ui.warpInput().clear(); }
            continue;
        }
        ...
        if (ui.mode() == UI::Mode::KARAOKE) {
            if (k == 'c' || k == 'C' || k == 27 || k == 'q') {
                ...
                else ui.setMode(UI::Mode::MAIN);
            } else if (k == ' ') { mp.togglePlay(); }
            else if (k == 'f')   { mp.seekSec(10);  ui.setStatus("Seek +10s"); }
            else if (k == 'B')   { mp.seekSec(-10); ui.setStatus("Seek -10s"); }
            else if (k == '+' || k == '=') { mp.volUp();   ui.setStatus("Vol " + std::to_string(mp.volume()) + "%"); }
            else if (k == '-' || k == '_') { mp.volDown(); ui.setStatus("Vol " + std::to_string(mp.volume()) + "%"); }
            else if (k == 'n')   { mp.next(); }
            else if (k == 'p')   { mp.prev(); }
            else if (k == 27) {
                int k2 = readKey();
        ...
        switch (k) {
            ...
            case 'z': case 'Z': mp.jumpRandom(); ui.setStatus("⤳ Random jump"); break;
            ...
            case 'c': case 'C': ui.setMode(UI::Mode::KARAOKE); break;
